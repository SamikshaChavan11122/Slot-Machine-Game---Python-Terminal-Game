    ...
    for line in range(lines):
        ...
        for column in columns:
            if column[line] != symbol:
                break
        else:  # This 'else' runs only if the loop wasn't broken
...
def get_slot_machine_spin(rows, cols, symbols):
    ...
    for symbol, count in symbols.items():
        all_symbols.extend([symbol] * count)  # More efficient way to add symbols
...
def print_slot_machine(columns):
    for row in range(len(columns[0])):
        for i, column in enumerate(columns):
            if i != len(columns) - 1:  # Corrected condition
...
def spin(balance):
    ...
    return balance - total_bet + winnings  # Update balance correctly
...
def main():
    ...
    while True:
        ...
        if answer.lower() == "q":
            break
        balance = spin(balance)  # Update balance correctly
